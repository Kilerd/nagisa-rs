...
mod tests {
    ...
    #[cfg_attr(
        ...
        ignore = "needs the CPython 3.12 dump: set RAGISA_UNICODE_REF (see README.md)"
    )]
    #[test]
    fn nfkc_and_lower_match_cpython312() {
        let path = std::env::var_os("RAGISA_UNICODE_REF").expect("unicode reference");
    ...
    /// a wrong table shows up. `RAGISA_UNICODE_SWEEPS` points at a
    ...
    #[cfg_attr(
        ...
        ignore = "needs the CPython 3.12 sweeps: set RAGISA_UNICODE_SWEEPS (see README.md)"
    )]
    #[test]
    fn nfkc_combining_sequences_match_cpython312() {
        let dir = std::env::var_os("RAGISA_UNICODE_SWEEPS").expect("unicode sweeps");
