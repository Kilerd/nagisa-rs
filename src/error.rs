//! Errors raised while loading nagisa data or preparing tokens.

use std::path::PathBuf;

/// Errors from loading a nagisa model or preparing already-segmented input.
#[derive(Debug, thiserror::Error)]
pub enum JaError {
    /// An input token became empty after normalization.
    #[error("token {index} is empty after normalization")]
    EmptyToken {
        /// Zero-based index of the empty token.
        index: usize,
    },
    /// Vocabulary IDs do not match the supported pretrained model.
    #[error("invalid vocabulary: {0}")]
    InvalidVocab(&'static str),
